...
$re_pass=isset($_POST["rnew-pass"]) ? $_POST["rnew-pass"] : "" ;//

/////////////////
$button=isset($_POST['sub-edit-per']) ? $_POST['sub-edit-per'] : "" ; //
...
if(isset($_POST["sub-edit-per"])){
    ...
    if ($check&&$Erorr_Flag_name==false&&$Erorr_Flag_email==false&&$Erorr_Flag_old_pass==false&&$Erorr_Flag_new_pass==false&&$Erorr_Flag_re_pass==false&&$Erorr_Flag_phone==false){
        if(isset($_POST['sub-edit-per'])){
            $query= "update users SET username='$name',email='$email',Phone='$phone',password='$new_pass' where ID='$id_per'";
